...
struct RomanSuggestView{
    static func expandedPath(rdw: CGFloat, ldw: CGFloat, width: CGFloat) -> some Shape {
        ...
        .scale(x: width/109, y: (Store.shared.design.keyViewSize.height*2+Store.shared.design.keyViewVerticalSpacing)/281, anchor: .top)
    }
    
    static func scaleToFrameSize(keyWidth: CGFloat, scale_y: CGFloat, color: Color) -> some View {
        let height = (Store.shared.design.keyViewSize.height*2 + Store.shared.design.keyViewVerticalSpacing) * scale_y
