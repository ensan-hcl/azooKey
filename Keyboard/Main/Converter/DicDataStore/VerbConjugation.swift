...
extension DicDataStore{
    private func 動詞情報照会(cid: Int) -> (活用: 活用の種類, 行の名前: 行)? {
        if cid == 583{
    ...
    private func 活用形取得(データ: (word: String, ruby: String, cid: Int), 活用: 活用の種類, 行の名前: 行) -> [(word: String, ruby: String, cid: Int)] {
        switch 活用{
            case .五段:
                //可能動詞は無視する
                switch データ.cid{
    ...
    func getConjugations(data: (word: String, ruby: String, cid: Int)) -> [(word: String, ruby: String, cid: Int)] {
