...
class KanaKanjierTests: XCTestCase {
    ...
    func testPerformanceBuildingTrie() throws {
        ...
        self.measure {
            ...
            do{
                guard let path = Bundle(for: type(of: self)).path(forResource: "キョ", ofType: "tsv") else {
                    print("ファイルが存在しません")
                    return
                }
                print("ファイルが存在しました")
    ...
    func testPerformanceBuildingTrie2() throws {
    ...
    func testPerformanceFindingTrie() throws {
        ...
        self.measure {
            let indices = trieroot.find(for: "キョウ")
            let data = indices.map{csvData[$0]}
            print(data.map{$0[1]})
        }
    }

    func testPerformancePrefixingTrie() throws {
        let csvData: [[String]]
        let trieroot = TrieNode<Character, Int>()
        ...
        self.measure {
            let indices = trieroot.prefix(for: "キョウ")
            let data = indices.map{csvData[$0]}
            print(data.map{$0[1]})
        }
    }

    func testPerformanceBuildingDict() throws {
        print()
        self.measure {
            var dict:[[String:[[String]]]] = [[String:[[String]]]].init(repeating: [:], count: 21)
            ...
            do{
                ...
                csvData.indices.forEach{index in
                    let ruby = csvData[index][0]
                    let rubyCount = ruby.count
                    dict[rubyCount][ruby, default:[]].append(csvData[index])
                }
            } catch let error {
                print("ファイルが存在しません: \(error)")
            }
    ...
    func testPerformanceFindingDict() throws {
        var dict:[[String:[[String]]]] = [[String:[[String]]]].init(repeating: [:], count: 21)
        ...
        self.measure {
            print(dict[3]["キョウ"]?.map{$0[1]})
        }
    }
    
    func testPerformancePrefixDict() throws {
        var dict:[[String:[[String]]]] = [[String:[[String]]]].init(repeating: [:], count: 21)
        ...
        self.measure {
            let data:[[String]] = dict.flatMap{dic -> [[String]] in
                let filtered = dic.filter{$0.key.hasPrefix("キョ")}
                return filtered.flatMap{$0.value}
            }
            print(data.map{$0[1]})
