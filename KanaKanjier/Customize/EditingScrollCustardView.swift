...
struct EditingScrollCustardView: View {
    private let base: [CustardKeyPositionSpecifier: CustardInterfaceKey] = [
        ...
        .grid_scroll(1): .custom(.init(design: .init(label: .systemImage("list.dash"), color: .special), press_actions: [.toggleTabBar], longpress_actions: [], variation: [])),
        .grid_scroll(2): .custom(.init(design: .init(label: .systemImage("delete.left"), color: .special), press_actions: [.delete(1)], longpress_actions: [.longDelete(1)], variation: [])),
    ...
    func makeCustard(data: UserMadeGridScrollCustard) -> Custard {
        ...
        for substring in data.words.split(separator: "\n"){
            ...
            keys[.grid_scroll(.init(keys.count))] = .custom(.init(design: .init(label: .text(label), color: .normal), press_actions: [.input(input)], longpress_actions: [], variation: []))
        }
