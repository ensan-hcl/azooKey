...
/*
extension CGImage{
    func fixed(_ imageOrientation: UIImage.Orientation) -> CGImage? {
        ...
        return autoreleasepool{() -> CGImage? in
            guard let colorSpace = self.colorSpace,
                  let ctx = CGContext(data: nil, width: self.width, height: self.height, bitsPerComponent: self.bitsPerComponent, bytesPerRow: 0, space: colorSpace, bitmapInfo: CGImageAlphaInfo.premultipliedLast.rawValue) else {
                return nil
            }
            ...
            debug("方向", imageOrientation)
            debug("変換前", self.width, self.height)
            debug("変換後", result?.width, result?.height)
